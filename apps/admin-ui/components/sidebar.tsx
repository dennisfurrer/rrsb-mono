...
export function Sidebar() {
  ...
  return (
    <aside className="w-72 md:w-64 lg:w-72 h-full bg-sidebar border-r border-border flex flex-col shrink-0 relative">
      ...
      {/* Logo + Brand */}
      <div className="shrink-0 border-b border-border px-4 py-2.5 md:py-4">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        <nav className="py-2 md:py-4 px-3 space-y-0.5 md:space-y-1" data-walkthrough="sidebar-nav">
          {filteredSections.map((section, sIdx) => (
            <div key={section.labelKey + sIdx}>
              {section.separatorBefore && (
                <div className="border-t border-border-strong mt-4 mb-4 md:mt-8 md:mb-8" />
              )}
              ...
              {section.href ? (
                <Link
                  href={section.href}
                  className={cn(
                    "flex items-center gap-3 px-3 py-1.5 md:py-2.5 rounded-lg text-sm transition-all duration-150 mb-0.5 md:mb-1",
                ...
                <>
                  {/* Section header */}
                  <div className="flex items-center gap-2 px-2 mb-1 mt-1 md:mb-1.5 md:mt-2">
                    {section.icon && (
                      <section.icon className={cn("w-3.5 h-3.5 shrink-0", section.dimmed ? "text-text-muted" : "text-text-secondary")} />
                    )}
                    <p className={cn(
                      "text-[0.65rem] md:text-[0.7rem] font-bold uppercase tracking-[0.15em]",
                  ...
                  <div className="space-y-0 md:space-y-0.5 pl-6 pr-6">
                    {section.items.map((item) => {
                      ...
                      if (item.disabled || section.dimmed) {
                        return (
                          <div
                            key={item.labelKey}
                            className="flex items-center gap-2.5 px-3 py-1 md:py-1.5 text-[0.65rem] md:text-[0.7rem] font-medium text-text-muted cursor-not-allowed select-none"
                          >
                            {t(item.labelKey)}
                      ...
                      if (section.compact) {
                        return (
                          <Link
                            ...
                            href={item.href}
                            className={cn(
                              "flex items-center gap-2.5 px-3 py-1 md:py-1.5 rounded-lg text-[0.65rem] md:text-[0.7rem] font-medium transition-all duration-150",
                              isActive
                                ? "text-accent"
                      ...
                      return (
                        <Link
                          ...
                          href={item.href}
                          className={cn(
                            "flex items-center gap-3 px-3 py-1.5 md:py-2.5 rounded-lg text-[0.82rem] md:text-[0.92rem] leading-[1.3rem] transition-all duration-150",
                            isActive
                              ? "bg-accent/[0.10] text-accent font-semibold shadow-[inset_0_0_0_1px_rgba(212,168,67,0.12)]"
                              : "text-text-primary/[0.92] hover:text-accent hover:bg-white/[0.04]"
                          )}
                        >
                          <Icon className={cn("w-[15px] h-[15px] md:w-[17px] md:h-[17px] shrink-0", isActive && "drop-shadow-[0_0_4px_rgba(212,168,67,0.3)]")} />
                          {t(item.labelKey)}
                        </Link>
                      );
                    })}
      ...
      <div className="shrink-0 p-3">
          ...
          <div className="flex items-center gap-0.5">
            {SUPPORTED_LOCALES.map((locale) => (
              <button
                ...
                className={cn(
                  "rounded-md px-2 py-1 text-[0.65rem] font-semibold uppercase tracking-wider transition-all duration-150",
        ...
        <div className="rounded-lg bg-white/[0.02] border border-border p-2.5 md:p-3">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 md:w-9 md:h-9 rounded-full bg-white/[0.04] flex items-center justify-center ring-1 ring-white/[0.06]">
              <span className="text-[0.65rem] md:text-xs font-semibold text-text-secondary">
                {initials}
              </span>
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-xs md:text-sm font-medium text-text-primary truncate">{user?.displayName}</p>
              <p className="text-[0.65rem] md:text-xs text-text-muted truncate">{user?.email}</p>
            </div>
          </div>
          <button
            onClick={logout}
            className="mt-2.5 md:mt-3 w-full flex items-center justify-center gap-2 rounded-md px-3 py-1.5 md:py-2 text-xs md:text-sm font-medium text-danger/80 hover:text-danger hover:bg-danger/[0.06] border border-danger/[0.06] hover:border-danger/10 transition-all duration-150"
          >
