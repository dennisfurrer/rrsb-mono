          ...
          return (
            <div style={{ width: "100%", background: "#111", borderRadius: "8px", padding: "0.5vh 0.3vw", position: "relative" }}>
              ...
              <svg viewBox={`0 0 ${svgW} ${svgH}`} preserveAspectRatio="none" style={{ width: "100%", height: "36vh", display: "block", overflow: "visible" }}>
                ...
                {(() => { const bw = Math.max(String(fs0).length, String(fs1).length) * 7 + 4; const fx = lastX + 5 + bw / 2; const s0c = String(fs0).length < String(fs1).length; const s1c = String(fs1).length < String(fs0).length; return <><text x={s0c ? fx : lastX + 7} y={rcFs0y} textAnchor={s0c ? "middle" : "start"} dominantBaseline="middle" fontSize={12} fill={c1}>{fs0}</text><text x={s1c ? fx : lastX + 7} y={rcFs1y} textAnchor={s1c ? "middle" : "start"} dominantBaseline="middle" fontSize={12} fill={c2}>{fs1}</text></>; })()}
              </svg>
            </div>
          );
        })();
